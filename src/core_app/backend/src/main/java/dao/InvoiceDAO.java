...
public class InvoiceDAO {
    ...
    public Invoice createInvoice(JsonObject jsonData) throws SQLException {
        ...
        try {
            ...
            Integer customerId = jsonData.has("customer_id") && !jsonData.get("customer_id").isJsonNull() 
                    ? jsonData.get("customer_id").getAsInt() : null;
            ...
            String sqlUpdateInventory = "UPDATE Inventory SET current_quantity = current_quantity - ? WHERE batch_id = ?";
            ...
            if (psUpdateInventory != null) psUpdateInventory.close();
            if (conn != null) {
     ...
     */
    public List<Invoice> getInvoices(String dateFrom, String dateTo, Integer pharmacistId, Boolean isSimulated) throws SQLException {
        List<Invoice> invoices = new ArrayList<>();
        StringBuilder sql = new StringBuilder(
            "SELECT i.invoice_id, i.invoice_date, i.branch_id, i.pharmacist_id, i.customer_id, " +
            "i.total_amount, i.is_simulated, " +
            "b.branch_name, p.full_name as pharmacist_name, c.full_name as customer_name " +
            "FROM Invoices i " +
    ...
    public JsonObject getInvoiceStats(String dateFrom, String dateTo) throws SQLException {
        ...
        StringBuilder sql = new StringBuilder(
            ...
            "SUM(CASE WHEN is_simulated = 0 THEN 1 ELSE 0 END) as real_count, " +
            "SUM(CASE WHEN is_simulated = 1 THEN 1 ELSE 0 END) as simulated_count " +
